...
int razer_debug_enabled = 0;

// Enhanced debug version of razer_send_payload
static struct razer_report razer_send_payload(IOUSBDeviceInterface **usb_dev, struct razer_report *request_report)
{
    ...
    if (razer_debug_enabled) {
        printf("=== SENDING COMMAND ===\n");
        printf("Command Class: 0x%02x\n", request_report->command_class);
        printf("Command ID: 0x%02x\n", request_report->command_id.id);
        printf("Data Size: %d\n", request_report->data_size);
        printf("Transaction ID: 0x%02x\n", request_report->transaction_id.id);
        
        printf("Request Data: ");
        for(int i = 0; i < request_report->data_size && i < 20; i++) {
            printf("%02x ", request_report->arguments[i]);
        }
        printf("\n");
        
        printf("Full Request Packet: ");
        unsigned char *packet = (unsigned char*)request_report;
        for(int i = 0; i < sizeof(struct razer_report); i++) {
            printf("%02x ", packet[i]);
            if((i + 1) % 16 == 0) printf("\n");
        }
        printf("\n========================\n");
    ...
    if (razer_debug_enabled) {
        ...
        if(retval == 0) {
            ...
            for(int i = 0; i < response_report.data_size && i < 20; i++) {
                printf("%02x ", response_report.arguments[i]);
            }
            printf("\n");
        }
        printf("================\n\n");
    ...
    if(retval == 0) {
        ...
        if(response_report.remaining_packets != request_report->remaining_packets ||
           ...
           response_report.command_id.id != request_report->command_id.id) {
            ...
            if (razer_debug_enabled) printf("Device is busy (mousedock)\n");
        ...
        } else if (response_report.status == RAZER_CMD_SUCCESSFUL && razer_debug_enabled) {
...
// Add a function to enable/disable debug from Node.js
void razer_set_debug(int enabled) {
    razer_debug_enabled = enabled;
    printf("Razer debug %s\n", enabled ? "enabled" : "disabled");
}
...
static ssize_t razer_attr_write_matrix_effect_static_common(struct device *dev, struct device_attribute *attr, const char *buf, size_t count, unsigned char led_id)
{
    ...
    case USB_DEVICE_ID_RAZER_BASILISK_V3:
    case USB_DEVICE_ID_RAZER_BASILISK_V3_X_HYPERSPEED:
        report = razer_chroma_extended_matrix_effect_static(VARSTORE, SCROLL_LED, (struct razer_rgb*)&buf[0]);
